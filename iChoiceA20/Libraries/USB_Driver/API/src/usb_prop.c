...
/***********************************************
* 功能：获得USB连接的状态
* 返回：USB接口状态DEVICE_STATE
***********************************************/
uint32_t USB_connectionState()
{
    return bDeviceState;   
}
...
{
    uint8_t i = 0;
    if(rxCounter == 0)//没有收到数据
        return 0;
    for(;i < rxCounter;i++)
        dataBuf[i] = rxBuffer[i + 2];//把收到的数据写入用户区
    rxCounter = 0;//清接收计数器
    SetEPRxStatus(ENDP1, EP_RX_VALID);//设置接收有效,等待下次数据
    return i;//返回收到的数据长度
}

uint16_t hidSendDataInBackground(uint8_t* dataBuf, uint16_t size, uint8_t intfNum, uint64_t ulTimeout)
{
    if(size > 62 || size == 0)
        return 0;
    while(!isSentComplete && ulTimeout > 0) ulTimeout--;//等待上次发送结束
    txBuffer[0] = 0x3F;//写入Report ID
    txBuffer[1] = (uint8_t)size;
    memcpy(&txBuffer[2],dataBuf,size);
    USB_SIL_Write(EP1_IN,txBuffer,64);//发送数据-数据必须发满64个字节
    SetEPTxValid(ENDP1);//设置端点有效,开始传递数据
    isSentComplete = 0;//置为未发送完状态
    return size;
}